...
public:
    ...
    PUBLIC_PROCEDURE(DistributeToken) if (qpi.invocationReward() < AIRDROP_TRANSER_FEE)
    {
        if (qpi.invocationReward() > 0)
        {
            qpi.transfer(qpi.invocator(), qpi.invocationReward());
    ...
    PUBLIC_PROCEDURE(TransferToken) if (qpi.invocationReward() < AIRDROP_TRANSER_FEE)
    {
        if (qpi.invocationReward() > 0)
        {
            qpi.transfer(qpi.invocator(), qpi.invocationReward());
